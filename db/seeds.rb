...
def create_saes(json)
  json.each do |rec|
    ...
    sae = Sae.find_or_create_by(name: rec["name"], short_name: rec["short_name"])
    rec["directions"].each { |direction| sae.specialties << Specialty.where(code: direction["code"], profile: direction["profiles"]) }
  end
end
